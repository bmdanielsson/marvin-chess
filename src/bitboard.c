...
/*
 * All magics used in this file were generated by Pradyumna (Pradu) Kannan.
 * http://pradu.us/old/Nov27_2008/Buzz/releases/magicmoves.zip
 */

/* Information about a magic */
struct magic {
   uint64_t *moves;
   uint64_t mask;
   uint64_t magic;
...
/* Magics for rooks */
static const uint64_t rook_magics[NSQUARES] = {
...
static const int rook_shifts[NSQUARES] = {
 ...
 *   Rxxxxxx.
 */
static const uint64_t rook_masks[NSQUARES] = {
...
/* Magics for bishops */
static const uint64_t bishop_magics[NSQUARES] = {
...
static const int bishop_shifts[NSQUARES] = {
 ...
 *   ..B.....
 */
static const uint64_t bishop_masks[NSQUARES] = {
 ...
 */
static uint64_t rook_moves_db[102400];
 ...
 */
static uint64_t bishop_moves_db[5258];

/* Table of magic information for rooks */
struct magic rook_magic_table[NSQUARES];

/* Table of magic information for bishops */
struct magic bishop_magic_table[NSQUARES];
...
/*
 * Initialize the magic bitboard databases for rooks and bishops. The
 * databases contains a bitboard with possible moves for each
...
static void init_magic_databases(void)
{
    ...
    memset(bishop_moves_db, 0, sizeof(bishop_moves_db));
    next_index = 0;
    iter = &bishop_moves_db[next_index];
    for (sq=0;sq<NSQUARES;sq++) {
        ...
        /* Initialize magic table for this square */
        bishop_magic_table[sq].moves = iter;
        bishop_magic_table[sq].mask = mask;
        bishop_magic_table[sq].magic = magic;
        bishop_magic_table[sq].shift = shift;
        ...
        for (k=0;k<nocc;k++) {
            ...
            moves |= get_slider_moves(sq, -1, -1, occ);
            moves |= get_slider_moves(sq, 1, -1, occ);
            index = (occ*magic)>>shift;
            max_index = MAX(max_index, index);
            iter[index] = moves;
        }

        /* Calculate the location in the database for the next square */
        next_index += (max_index + 1);
        iter = &bishop_moves_db[next_index];
    }

    /* Populate the rook database */
    memset(rook_moves_db, 0, sizeof(rook_moves_db));
    next_index = 0;
    iter = &rook_moves_db[next_index];
    for (sq=0;sq<NSQUARES;sq++) {
        /* Setup this iteration */
        nblockers = 0;
        mask = rook_masks[sq];
        ...
        /* Initialize magic table for this square */
        rook_magic_table[sq].moves = iter;
        rook_magic_table[sq].mask = mask;
        rook_magic_table[sq].magic = magic;
        rook_magic_table[sq].shift = shift;
...
uint64_t bb_bishop_moves(uint64_t occ, int from)
{
    ...
    ptr = &bishop_magic_table[from];
    index = occ&ptr->mask;
    index *= ptr->magic;
    index >>= ptr->shift;

    return ptr->moves[index];
}
